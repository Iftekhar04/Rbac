...
const UserPage = () => {
  ...
  return (
    <div className="relative w-full px-5 pt-5 py-5">
      <h1 className="text-3xl font-bold">Users</h1>
      <div className="flex gap-5">
        <div className="py-2 px-3 bg-[#E8EDF5] w-[92%] my-5 flex items-center gap-3 rounded-lg">
          <CiSearch className="size-6 text-[#4A789C]" />
          <input
            type="text"
            placeholder="Search by name, email or role"
            className="bg-[#E8EDF5] w-[100%] text-[#4A789C] outline-none"
          />
        </div>
        <button
          className="p-2 bg-[#E8EDF5] rounded-lg my-5 text-[#4A789C]"
          ...
          {users.map((user) => (
            <tr className="border border-b-2 border-solid" key={user.id}>
              ...
              <td className="text-start ps-4 flex gap-3 py-2">
                <FaRegEdit className="size-5" onClick={() => setSelectedUser(user)}/>{" "}
                <MdOutlineDelete className="size-5" onClick={() => setUsers(users.filter(u => u.id !== user.id))}/>
              </td>
            </tr>
          ))}
        </tbody>
